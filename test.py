...
class BabyMonitoringSystem:
            ...
            for det in pred:
                if len(det):
                    ...
                    for *xyxy, conf, cls in det:
                        ...
                        detections.append({"label": label, "confidence": float(conf), "bbox": [int(x) for x in xyxy]})
    ...
    def detect_faces(self, frame):
        ...
        for face in faces:
            ...
            face_results.append({"bbox": bbox, "is_known": is_known, "name": name, "similarity": similarity})
                    ...
                    if pose_info["pose_detected"]:
                        ...
                        if pose_info["danger_level"] == "high":
                            if current_time - self.last_sent_time >= SEND_INTERVAL:
                                ...
                                self.send_to_thingspeak("dangerous_pose", pose_info["confidence"], danger_type)
                                self.last_sent_time = current_time
                                print("🚨 POSE CẢNH BÁO: Tư thế nguy hiểm - Đã gửi thông báo!")
                        ...
                        if not face["is_known"]:
                            ...
                            if current_time - self.last_sent_time >= SEND_INTERVAL:
                                self.send_to_thingspeak(
                                    "stranger_detected", face["similarity"], f"Unknown_person_{face['similarity']:.2f}"
                                )
                                self.last_sent_time = current_time
