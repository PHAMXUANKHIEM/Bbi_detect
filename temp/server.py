...
ZAPIER_WEBHOOK_URL = "https://hooks.zapier.com/hooks/catch/22976197/27fmde4/"

# === GOOGLE DRIVE SETUP ===
SCOPES = ["https://www.googleapis.com/auth/drive.file"]
GOOGLE_DRIVE_FOLDER_ID = "1kTGjnNWWKZm2gGfG1QpDfzrWX8qrZAJA"  # hoặc ID thư mục bạn muốn lưu ảnh
...
# === LOGGING ===
logging.basicConfig(
    filename="face_recognition.log", level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s"
)
...
if not os.path.exists(known_faces_dir):
    print(f"❌ Thư mục {known_faces_dir} không tồn tại.")
    exit()
...
esp32_stream_url = "http://192.168.184.109/stream"
...
def send_thingspeak_alert():
    try:
        res = requests.get(THINGSPEAK_URL, params={"api_key": THINGSPEAK_WRITE_API, "field1": 1}, timeout=5)
...
def send_zapier_webhook(image_link):
    payload = {
        "message": "Phát hiện người lạ từ ESP32-CAM!",
            ...
            if True in matches:
                ...
                if current_time - last_alert_time > alert_interval:
                    ...
                    if send_thingspeak_alert():
                        ...
                        print(f"🔗 Đường dẫn ảnh Google Drive: {drive_link}")
